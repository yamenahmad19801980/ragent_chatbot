...
Refactored to use centralized DeviceService.
"""
...
class DeviceTools:
    """Tools for controlling IoT devices using centralized DeviceService."""
    
    def __init__(self, api_client: SyncrowAPIClient):
        self.device_service = DeviceService(api_client)
...
def create_device_tools(api_client: SyncrowAPIClient) -> List[Tool]:
    ...
    def control_device_tool(device_uuid: str, user_message: str, product_type: str) -> str:
        ...
        result = device_tools.device_service.control_device(device_uuid, user_message, product_type)
        return str(result)
    
    def query_device_tool(device_uuid: str) -> str:
        """Tool for querying device status."""
        result = device_tools.device_service.query_device_status(device_uuid)
        return str(result)
    
    def schedule_device_tool(device_uuid: str, user_message: str) -> str:
        """Tool for scheduling device actions."""
        result = device_tools.device_service.schedule_device(device_uuid, user_message)
        return str(result)
